...
for f in files:
    ...
    previous_X = X
    # Center of new global_pose is C = -inv(global_R) @ global_T = global_R.T @ global_T  
    # position = - global_pose[:3,:3].T @ global_pose[:3,-1]
    position = global_pose[:3,-1]
    traj.append(position) 

    # x = np.array([(global_pose[:3,:3] @ k[:3]-global_pose[:3,-1]).flatten() for k in X])
    # plotx =[]
    # for i in x:
    #     if i[2]>=0:
    #         plotx.append(i)
    # plotx = np.array(plotx)
    # plt.scatter(plotx[:10,0], plotx[:10,2], color= 'green', s =1)
